...
By default, Nextflow will generate a `work` directory in the working directory from which it is executed, typically the `nextflow_cutNrun` directory. These folders contains all subsequent steps in the pipeline, and to understand their application and the finer points of pipeline troubleshooting please see the [Nextflow documentation](https://www.nextflow.io/docs/latest/cache-and-resume.html). These files should not be necessary beyond troubleshooting and caching, and they can be safely deleted once a pipeline has been completed to free up disk space. Note however that once removed (either via `rm -r` or `nextflow clean`), subsequent pipeline runs cannot be `-resume`d and will thus *repeat all processes and replace files in the `--out_dir` directory with fresh copies*. This is because the `work` directory files are tracked for caching, *not* the files in the output directory. For this reason, it is recommended to retain the `work` folder until you are confident you have what you need from the process.
