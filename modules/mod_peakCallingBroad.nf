...
process peakCallingBroad {
  ...
  output:
    tuple val(sys_idx), val(samp_idx), val(cond_name), val(proj), val(samp_name), val(cell_line), val(epitope), val(cond), val(rep), path("${samp_name}_peaks.broadPeak"), emit: "broadPeaks"
    path "*.txt"
    path "*.tsv"
