...
process peakCallingBroadPooled{
  ...
  input:
    tuple val(samp_name), val(cell_line), val(epitope), val(condition), path(bams_input), path(bams_ctrl)
    path blacklist_bed
    path seqsize_tsv
  
  output:
    tuple val(samp_name), val(cell_line), val(epitope), val(condition), path("${samp_name}_peaks.broadPeak"), emit: "broadPeaks"
    path "*.txt"
    path "*.tsv"
